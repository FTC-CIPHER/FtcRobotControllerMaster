...
@TeleOp(name="LinearSlideTesting2")
public class LinearSlideTesting2 extends LinearOpMode{
    ...
    double RunToLinearPos = 0;
    double TargetLinearPosition = 0;
    double RunToLateralPos = 0;
    ...
    boolean Gamepad1DpadUp = false;
    boolean Gamepad1DpadDown = false;
    boolean RunningToLinearPos = false;
    ...
    private DcMotor driveMotorFrontLeft;
    ...
    public void runOpMode() {
        ...
        driveMotorBackLeft.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        ...
        while(opModeIsActive()){
            ...
            if(RunningToLinearPos){
                if(Math.abs(TargetLinearPosition) > Math.abs(driveMotorBackLeft.getCurrentPosition())){
                    GyroYaw = 0;
                    GamepadLeftX = (RunToLinearPos/(Math.abs(RunToLinearPos)))*0.2;
                }else{
                    RunningToLinearPos = false;
                }
            }
            ...
            ViperSlidel.setPosition(ViperSlideLeftCurrentPosition);
            ViperSlider.setPosition(ViperSlideRightCurrentPosition);
            ...
            telemetry.addData("GamepadLeft", gamepad1.left_stick_x);
            telemetry.addData("Test3", TargetLinearPosition);
            telemetry.addData("test2", RunningToLinearPos);
            ...
            telemetry.addData("testing", driveMotorBackLeft.getCurrentPosition());
            ...
            telemetry.addData("BackClawLeft", ViperSlidel.getPosition());
            telemetry.addData("BackClawRight", ViperSlider.getPosition());
    ...
    public void LinearMovement(double LinearMove){
        RunningToLinearPos = true;
        RunToLinearPos = LinearMove / 0.002207941063906578;
        TargetLinearPosition = RunToLinearPos + driveMotorBackLeft.getCurrentPosition();
    }
    public void LateralMovement(double LateralMove){
        RunToLateralPos = LateralMove * 0.001;
        LinearSlideBackLeftCurrentPosition += RunToLateralPos;
        LinearSlideBackRightCurrentPostion -= RunToLateralPos;
    }
}
