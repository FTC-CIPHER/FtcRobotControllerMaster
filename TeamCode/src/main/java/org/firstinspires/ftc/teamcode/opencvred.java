...
public class opencvred extends LinearOpMode {
    ...
    class YellowBlobDetectionPipeline extends OpenCvPipeline {
        ...
        private Mat preprocessFrame(Mat frame) {
            ...
            Scalar lowerRed = new Scalar(0, 60, 230);
            Scalar upperRed = new Scalar(24, 255, 255);
            Scalar otherlowerRed = new Scalar(325, 60, 230);
            Scalar otherupperRed = new Scalar(360, 255, 255);

            Mat redMask = new Mat();
            Core.inRange(hsvFrame, lowerRed, upperRed, redMask);
            Core.inRange(hsvFrame, otherlowerRed, otherupperRed, redMask);
            Mat kernel = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(10, 10));
            Mat erodedRed = new Mat();
            Imgproc.erode(redMask, erodedRed, kernel);
            Imgproc.morphologyEx(erodedRed, erodedRed, Imgproc.MORPH_OPEN, kernel);
